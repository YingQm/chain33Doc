...
**安装命令:**
安装npm和node.js（详见https://docs.npmjs.com/getting-started/installing-node），Remix-ide已经作为npm的模块：
...
>	具体操作可以参照：https://github.com/ethereum/remix-ide
...
## 1.4 调用JSONRPC接口部署智能合约  


**1** 使用solidity创建合约时生成的input code，调用相关JSONRPC接口，将合约部署到chain33链上,下面介绍相关JSONRPC接口，其中解锁钱包Chain33.Unlock我使用python调用接口，我将python调用方法展示在下面，限于篇幅不一一列举q所有jsonrpc的使用：
...
执行完返回一串hash值。这个hash值，加上user.p.developer.user.evm.这个前缀构成了这个游戏合约的地址，如下所示：
