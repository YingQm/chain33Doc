...
根据Chain33的RPC接口定义，可以通过编程，和基于Chain33构建的区块链系统进行通信交互，提交交易，查询交易，调用智能合约，查询智能合约数据等。
下面分别针对Go语言和Python语言给出几个程序样例，用于展示如何对Chain33的RPC接口进行编程。
...
chain33节点收到请求后，进行处理：  
（1）如果是查询数据的请求，则从本地的区块信息、交易信息中获取相关信息，并返回给应用系统。  
（2）如果是提交交易的请求，则将交易信息广播到chain33区块链网络中，被区块链网络进行共识、打包等处理，并把相关信息记录到区块链上，同时向应用系统返回交易hash，应用系统可以根据该hash查询交易的执行结果。  

### 2. Chain33区块链正常工作  

为了进行RPC接口编程测试，需要有一个提供json rpc接口服务的chain33节点。  
对于学习chain33的开发者来说，具体可以参见“快速开始”章节，快速构建一个solo单节点的区块链，这个节点同时提供json rpc服务可供调用。  
也可以参考“环境部署”章节，基于一个构建好的区块链，从中选择一个节点提供json rpc服务可供调用。  
...
如果用go语言开发，则需要准备好go语言开发环境，可参考“环境准备”章节的go环境安装。  
如果用python语言开发，则需要准备好python语言开发环境，可从互联网上搜索python安装。  

## 二、Chain33 RPC接口的Go语言编程样例

### 1. go语言编程样例1：在不同账户间转账交易
...
其中参数1CbEVT9RnM5oZhWMj4fxUrJX94VtRotzvs为转出账户地址，1QJ6WX4W8o8UhVwEapis1P4wyoB9PrD4Nx为转入账户地址，3990969DF92A5914F7B71EEB9A4E58D6E255F32BF042FEA5318FC8B3D50EE6E8为转出账户地址私钥，可根据具体情况调整。  
...
其中参数0xda602f79fa4ff738441d1f53f56287bff8f48e9509d51a62be91e0fa664a8ed2为某一个交易的hash值，可根据具体情况调整。  
