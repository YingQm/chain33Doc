...
## 1.8 获取区块的序列信息 GetBlockSequences
...
|isHeader|bool|推送的数据类型；false:block 或者 true:header|
|type|int32|类型，0：推送 Block 信息,1：推送 BlockHeader 信息,2：推送交易回执|
|contract|map[string]bool|-|

<!--
|lastSequence|int64|推送不是从零开始|
|lastHeader|int64|推送不是从零开始r|
|lastBlockHash|string|推送不是从零开始|

last三个参数有三种组合可以使用
 1. lastSequence = lastHeader = 0, lastBlockHash = “”， 从零开始推送
 1. lastSequence = -1,  lastHeader， lastBlockHash 指定特定的值，从 lastHeader， lastBlockHash 指定的值开始推送。 有些业务可能是从某高度开始推送的， 不知道 对应的sequence
 1. lastSequence lastHeader lastBlockHash 指定特定的值， 从指定的值开始推送
-->
...
```json
{
	...
	"result": {
		...
		"seqs": [
        {
            "Hash": "0xe15fb9d8ee52be4654d7250266cdf8c86e84f6d0518ad81699b3d81e71fc3828",
            "Type": 1,
            "sequence": 6,
            "height": 6
        },
        {
            "Hash": "0xe7cafe9bfbb7e9230b4c5b3c080c75ce73e9018d48e54e32e26e7232440eb060",
            "Type": 1,
            "sequence": 5,
            "height": 5
        }]
...
|isOK|bool|是否设置成功|
|msg|string|错误信息|
|seqs| seq 数组| 在注册情况 2,3 时， 指定的点不是节点的有效点(即可能在分叉上， 或参数写错了)，会返回推荐的注册点，再次注册时，可以从中选择参数 |
|seqs.Hash|string| 区块hash|
|seqs.Type|int| 1 为Add， 2为Delete|
|seqs.sequence| int | 区块sequence |
|seqs.height|int| 区块height|
...
## 1.11 列举区块序列号回调 ListSeqCallBack
...
## 1.12 获取某回调最新序列号的值 GetSeqCallBackLastNum
